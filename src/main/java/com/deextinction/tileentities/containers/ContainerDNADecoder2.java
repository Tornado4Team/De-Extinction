...
public class ContainerDNADecoder2 extends ContainerMachine
{
	private TileDNADecoder dnaDecoder;
	private int decodingProgress;
	private int researchProgress;
	private int isAllowedToDecode;

	public ContainerDNADecoder2(InventoryPlayer playerInventory, TileDNADecoder dna_decoder)
	{
		this.dnaDecoder = dna_decoder;

		// TileEntity inventory
		this.addSlotToContainer(new Slot(this.dnaDecoder, TileDNADecoder.SLOT_INDEX_DNA_BOTTLE, 38, 32)
		{
			@Override
		...
		this.addSlotToContainer(new Slot(this.dnaDecoder, TileDNADecoder.SLOT_INDEX_DISK_EMPTY, 73, 32)
		{
			@Override
			public boolean isItemValid(ItemStack stack)
			{
		...
		for (int x = 0; x < 2; x++)
			for (int y = 0; y < 2; y++)
				this.addSlotToContainer(new Slot(this.dnaDecoder, 2 + x + y * 2, 110 + x * 18, 34 + y * 18)
				{
					@Override
					public boolean isItemValid(ItemStack stack)
					{
		...
		// Player inventory
		for (int i = 0; i < 9; i++)
			this.addSlotToContainer(new Slot(playerInventory, i, 8 + i * 18, 174 - 24));

		for (int i = 0; i < 3; i++)
			for (int k = 0; k < 9; k++)
				this.addSlotToContainer(new Slot(playerInventory, k + i * 9 + 9, 8 + k * 18, 174 - 82 + i * 18));
	}
	...
	public void detectAndSendChanges()
	{
		...
		for (int i = 0; i < this.listeners.size(); ++i)
		{
			...
			if (this.isAllowedToDecode != this.dnaDecoder.getField(2))
		...
		this.isAllowedToDecode = this.dnaDecoder.getField(2);
	}
