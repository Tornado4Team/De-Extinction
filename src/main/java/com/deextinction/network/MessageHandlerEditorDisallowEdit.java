...
public class MessageHandlerEditorDisallowEdit implements IMessageHandler<MessageEditorDisallowEdit, IMessage>
{
	public IMessage onMessage(final MessageEditorDisallowEdit message, MessageContext ctx)
	{
	...
	private void processMessage(MessageEditorDisallowEdit message, EntityPlayerMP sendingPlayer)
	{
		...
		if (tileEntity != null)
		{
			if (tileEntity instanceof TileDNAEditor)
			{
				...
				dnaEditor.setAllowedToEdit(false);
			}
			else
				DeExtinction.logger.error("Wrong instance of tile entity when MessageEditorAllowEdit was received.");
		}
		else
			DeExtinction.logger.error("Null instance of tile entity when MessageEditorAllowEdit was received.");
	}
}
