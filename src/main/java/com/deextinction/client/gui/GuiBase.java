...
public abstract class GuiBase extends GuiContainer
{
	...
	{
		int buttonWidth = this.fontRenderer.getStringWidth(buttonName) + 10;
	...
	{
		int buttonHalfWidth = this.fontRenderer.getStringWidth(buttonName) / 2 + 5;
	...
	{
		if (text != null)
			this.fontRenderer.drawString(text, xPos, yPos, color);
	}
	...
	protected void writeCenteredText(String text, int xPos, int yPos, int color)
	{
		if (text != null)
		{
			int textHalfWidth = this.fontRenderer.getStringWidth(text) / 2;
			this.fontRenderer.drawString(text, xPos - textHalfWidth, yPos, color);
		}
	}
	...
	protected void writeCenteredTextInGui(String text, int yPos, int color)
	{
		...
		{
			int textHalfWidth = this.fontRenderer.getStringWidth(text) / 2;
			this.fontRenderer.drawString(text, this.xSize / 2 - textHalfWidth, yPos, color);
		}
	}
	...
	protected void writeTextWithScale(String text, int xPos, int yPos, int color, float scale)
	{
		if (text != null)
		{
			...
			this.fontRenderer.drawString(text, (int) (xPos / scale), (int) (yPos / scale), color);
	...
	protected void writeCenteredTextWithScale(String text, int xPos, int yPos, int color, float scale)
	{
		...
		{
			GL11.glScalef(scale, scale, 1.0F);
			this.fontRenderer.drawString(text, (int) (xPos / scale) - this.fontRenderer.getStringWidth(text) / 2, (int) (yPos / scale), color);
			GL11.glScalef(1 / scale, 1 / scale, 1.0F);
		}
	}
	...
	protected void writeCenteredTextInGuiWithScale(String text, int xPos, int yPos, int color, float scale)
	{
		...
		{
			GL11.glScalef(scale, scale, 1.0F);
			this.fontRenderer.drawString(text, (int) (this.xSize / (2 * scale)) - this.fontRenderer.getStringWidth(text) / 2, (int) (yPos / scale), color);
			GL11.glScalef(1 / scale, 1 / scale, 1.0F);
		}
	}
	...
	protected List<String> getTextInLinesStringSize(String text, int lineSize)
	{
		if (text != null && text != "")
		{
			...
			for (int index = 0; index < text.length(); index++)
			{
				...
				{
					String word = text.substring(lastIndex, index + 1);
					if (this.fontRenderer.getStringWidth(line + " " + word) < lineSize)
						line += word + " ";
					else
					{
						if (!line.isEmpty())
	...
	protected List<String> getTextInLinesJustifiedStringSize(String text, int lineSize)
	{
		if (text != null && text != "")
		{
			...
			{
				List<String> final_text = new ArrayList<String>();
				int spaceSize = this.fontRenderer.getStringWidth("A");
				for (int i = 0; i < text_lines.size() - 1; i++)
				{
					String line = text_lines.get(i);
					int missingSpaces = (lineSize - this.fontRenderer.getStringWidth(line)) / spaceSize;
					if (missingSpaces > 0)
						for (int index = 0; index < line.length(); index++)
						{
							Character charAt = line.charAt(index);
