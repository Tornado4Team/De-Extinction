...
public class GuiStringButton extends GuiButton
{
	...
	{
		super(id, xPos - (mc.fontRenderer.getStringWidth(displayString) + 2 * extraSpaceX) / 2, yPos, mc.fontRenderer.getStringWidth(displayString) + 2 * extraSpaceX, 12, displayString);
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks)
	{
		if (this.visible && this.displayString != null)
		{
			...
			{
				if (this.hovered)
					fontrenderer.drawString(this.displayString, this.x + this.extraSpaceX, this.y + 2, this.selectedColor);
				else
					fontrenderer.drawString(this.displayString, this.x + this.extraSpaceX, this.y + 2, this.normalColor);
			}
			else
				fontrenderer.drawString(this.displayString, this.x + this.extraSpaceX, this.y + 2, this.disabledColor);
