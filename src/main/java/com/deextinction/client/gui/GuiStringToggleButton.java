...
public class GuiStringToggleButton extends GuiButton
{
	 ...
	 */
	public GuiStringToggleButton(int id, int xPos, int yPos, int extraSpaceX, boolean state, String firstDisplayString, String secondDisplayString, int normalColor, int selectedColor, int disabledColor, Minecraft mc)
	{
		super(id, xPos - (mc.fontRenderer.getStringWidth(firstDisplayString) + 2 * extraSpaceX) / 2, yPos, mc.fontRenderer.getStringWidth(firstDisplayString) + 2 * extraSpaceX, 12, firstDisplayString);
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks)
	{
		if (this.visible)
		{
			...
			{
				if (this.hovered)
					fontrenderer.drawString(text, this.x + this.extraSpaceX, this.y + 2, this.selectedColor);
				else
					fontrenderer.drawString(text, this.x + this.extraSpaceX, this.y + 2, this.normalColor);
			}
			else
				fontrenderer.drawString(text, this.x + this.extraSpaceX, this.y + 2, this.disabledColor);
	...
	public void setFirstDisplayString(Minecraft mc, String text)
	{
		if (!text.isEmpty())
		{
			this.width = mc.fontRenderer.getStringWidth(text) + 2 * this.extraSpaceX;
	...
	{
		this.width = mc.fontRenderer.getStringWidth(this.displayString) + 2 * this.extraSpaceX;
		this.x = this.initPosX - this.width / 2;
	}
	...
	{
		this.width = mc.fontRenderer.getStringWidth(this.secondDisplayString) + 2 * this.extraSpaceX;
		this.x = this.initPosX - this.width / 2;
	}
